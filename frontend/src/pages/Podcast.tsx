...
export default function PodcastPage() {
  ...
  const togglePlay = () => {
    ...
    if (audio.playing && podcastData?.id) {
      api.put(`/podcasts/${podcastData.id}`, { last_position: audio.currentTime }).catch(() => { });
    }
  };
  ...
  const saveProgress = async (pos: number) => {
    if (podcastData?.id) {
      try { await api.put(`/podcasts/${podcastData.id}`, { last_position: pos }); } catch { }
    }
  };
...
const GFG_TOPICS = [
  "Big O Notation", "Recursion", "Dynamic Programming", "Binary Search",
  "Neural Networks", "Backpropagation", "Gradient Descent", "Transformer Architecture",
  "Attention Mechanism", "LSTM", "Decision Tree", "Support Vector Machine",
];
function GFGBadge({ topic }: { topic: string }) {
  const slug = topic.toLowerCase().replace(/[^a-z0-9 ]/g, "").replace(/\s+/g, "-");
  return (
    <a href={`https://www.geeksforgeeks.org/${slug}/`} target="_blank" rel="noopener noreferrer"
      ...
      <ExternalLink className="w-2 h-2" />{topic}
    </a>
  );
}
...
function NotesPanel({ paperId, paperTitle }: { paperId?: number, paperTitle?: string }) {
  ...
  // ── Tab 1 state ──────────────────────────────────────────────────────────
  const [notes, setNotes] = useState("<h2>Key Insights</h2><ul><li>Start typing your research notes here while listening…</li></ul>");
    ...
    >
      ...
      <div className="flex gap-0.5 p-1 bg-muted/40 border-b border-border">
        {(['notes', 'summary'] as const).map(t => (
          <button
            key={t}
            onClick={() => { setActiveTab(t); if (t === 'summary' && !summary) fetchSummary(); }}
      ...
      {activeTab === 'notes' && (
        <div className="flex flex-col flex-1 p-4 gap-3 overflow-y-auto">
          ...
          <div className="flex items-center justify-between flex-wrap gap-1">
            <h4 className="font-display font-semibold text-foreground text-sm">Research Notepad</h4>
            <div className="flex items-center gap-1">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="center" className="w-40 z-50">
                  <DropdownMenuItem onClick={() => execCommand('heading', 1)} className="cursor-pointer">H1 Title</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => execCommand('heading', 2)} className="cursor-pointer">H2 Subtitle</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => execCommand('heading', 3)} className="cursor-pointer">H3 Heading</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => execCommand('heading', false)} className="cursor-pointer">Body Text</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              <button onClick={() => execCommand('bold')} className="p-1 hover:bg-muted rounded"><Bold className="w-3 h-3" /></button>
              <button onClick={() => execCommand('list', 'bullet')} className="p-1 hover:bg-muted rounded"><List className="w-3 h-3" /></button>
              <button onClick={() => execCommand('list', 'ordered')} className="p-1 hover:bg-muted rounded"><ListOrdered className="w-3 h-3" /></button>
              <button onClick={handleCopy} className="p-1 hover:bg-muted rounded"><Copy className="w-3 h-3" /></button>
              <button
          ...
          {/* Quill editor */}
          <div className="quill-wrapper overflow-hidden bg-muted/10 rounded-xl border border-border" style={{ minHeight: 180 }}>
            <ReactQuill
              ref={quillRef}
              theme="snow"
              value={notes}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-44 z-50">
                <DropdownMenuItem onClick={() => handleDownload('md')} className="cursor-pointer text-sm">Markdown (.md)</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleDownload('doc')} className="cursor-pointer text-sm">Word Doc (.doc)</DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleDownload('pdf')} className="cursor-pointer text-sm">PDF (.pdf)</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
          ...
          <div className="border-t border-border pt-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-foreground">Research Flowchart</span>
                {flowNodes.length > 0 && (
                  <span className="text-[9px] text-muted-foreground">({flowNodes.length} steps)</span>
                )}
              </div>
              <button
            ...
            {flowLoading ? (
              <div className="py-8 flex flex-col items-center gap-2">
                ...
                <p className="text-[11px] text-muted-foreground">AI is building your flowchart…</p>
              </div>
            ) : flowNodes.length > 0 ? (
              <div className="space-y-0 max-h-72 overflow-y-auto pr-1">
                {flowNodes.map((node, i) => (
                  <div key={node.id || i}>
                    <div className={`rounded-xl border p-2.5 text-xs ${node.color || 'border-primary/30 bg-primary/5'}`}>
                      <span className={`text-[9px] font-bold uppercase ${node.labelColor || 'text-primary'}`}>{String(i + 1).padStart(2, '0')}</span>
                      <p className={`font-bold text-[11px] mt-0.5 ${node.labelColor || 'text-primary'}`}>{node.label}</p>
                      <p className="text-foreground text-[10px] leading-relaxed mt-0.5">{node.desc}</p>
                    </div>
      ...
      {activeTab === 'summary' && (
        <div className="flex flex-col flex-1 p-4 gap-3 overflow-y-auto">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-foreground">Paper Summary</span>
            <button
          ...
          {summaryLoading ? (
            ...
            <>
              ...
              {(() => {
                const topics = GFG_TOPICS.filter(t => summary.toLowerCase().includes(t.toLowerCase())).slice(0, 4);
                return topics.length > 0 ? (
                  <div className="flex flex-wrap gap-1 p-2 rounded-lg bg-green-500/5 border border-green-500/15">
                    <span className="text-[9px] font-bold text-green-600 w-full">📚 GFG References</span>
                    {topics.map(t => <GFGBadge key={t} topic={t} />)}
                  </div>
                ) : null;
              })()}
              <div
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-2 py-10 text-center">
              ...
              <p className="text-xs text-muted-foreground">Click Refresh to load the AI summary<br />for the current paper.</p>
            </div>
          )}
        </div>
      )}
