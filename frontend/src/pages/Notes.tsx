...
import {
    Search, FileText, Calendar, Plus, Trash2,
    Download, Sparkles, FileDown, BookOpen,
    Loader2, ExternalLink, Copy, ArrowDown, GitBranch
} from "lucide-react";
...
const GFG_TOPICS = [
    ...
    "Binary Search", "Sorting Algorithms", "Graph Theory", "Neural Networks",
    "Backpropagation", "Gradient Descent", "Transformer Architecture",
    "Attention Mechanism", "Matrix Multiplication", "Probability",
    "Bayesian Inference", "Entropy", "Convolutional Neural Network",
    "Recurrent Neural Network", "LSTM", "Decision Tree", "Random Forest",
    "Support Vector Machine", "Naive Bayes", "K-Means Clustering",
];
...
function extractGFGTopics(text: string) {
    return GFG_TOPICS.filter(p => text.toLowerCase().includes(p.toLowerCase())).slice(0, 6);
}
...
function AINotesTab() {
    ...
    useEffect(() => { fetchNotes(); loadFlowchart(); }, []);
    ...
    const handleCreateNote = async () => {
        ...
        } catch { toast({ title: "Error", description: "Failed to create note", variant: "destructive" }); }
    };
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            await api.put(`/notes/${editingNote.id}`, { title: editingNote.title, content: markdown });
            setNotes(prev => prev.map(n => n.id === editingNote.id
                ? { ...n, title: editingNote.title, content_preview: markdown.substring(0, 100) } : n));
            setEditingNote(null);
            toast({ title: "Saved ✓" });
    ...
    const handleExport = async (format: 'pdf' | 'doc' | 'md') => {
        ...
        if (format === 'md') {
            ...
            el.href = URL.createObjectURL(new Blob([turndownService.turndown(html)], { type: 'text/markdown' }));
            el.download = filename;
            document.body.appendChild(el); el.click(); document.body.removeChild(el);
            toast({ title: "Downloaded as Markdown" });
        } else if (format === 'doc') {
            const src = 'data:application/vnd.ms-word;charset=utf-8,' + encodeURIComponent(
                `<html><head><meta charset='utf-8'></head><body>${html}</body></html>`
            );
            const el = document.createElement("a");
            document.body.appendChild(el); el.href = src; el.download = filename; el.click(); document.body.removeChild(el);
            ...
            <div className="space-y-4">
                <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
                    <div>
                        ...
                        <p className="text-muted-foreground text-xs mt-0.5">Write, save, and export your research notes</p>
                    </div>
                    <div className="flex gap-2 flex-wrap">
                        ...
                        <button onClick={handleCreateNote} className="btn-primary py-2 px-4 flex items-center gap-2 text-sm">
                    ...
                    <AnimatePresence>
                        {filteredNotes.length > 0 ? filteredNotes.map((note, i) => (
                            <motion.div
                                ...
                                onClick={() =>
                                    api.get(`/notes/${note.id}`).then(({ data }) =>
                                        setEditingNote({ ...data, content: marked.parse(data.content || "") as string })
                                    )
                                }
                            >
                                ...
                                <h4 className="font-semibold text-foreground mb-1.5 line-clamp-2 text-sm">{note.title || "Untitled"}</h4>
                                <p className="text-muted-foreground text-xs line-clamp-3 flex-1 leading-relaxed">{note.content_preview || "No preview."}</p>
                            ...
                            <div className="col-span-full py-16 text-center text-muted-foreground">
                                ...
                                <p className="text-sm">No notes yet — click "New Note" to start.</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
            ...
            <div className="card-premium p-5">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-display font-semibold text-foreground">Research Flowchart</h3>
                    </div>
                    <button
                    ...
                    >
                        {flowLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                        Refresh
                    </button>
                </div>
                ...
                {flowLoading ? (
                    ...
                    <div className="space-y-0 max-w-xl mx-auto">
                        {flowTitle && (
                            <p className="text-center text-xs text-muted-foreground mb-4 font-medium">{flowTitle}</p>
                        )}
                        {flowNodes.map((node, i) => (
                            ...
                            >
                                <div className={`rounded-2xl border-2 p-4 ${node.color || "border-primary/40 bg-primary/5"}`}>
                                    <div className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${node.labelColor || "text-primary"}`}>
                                        {String(i + 1).padStart(2, "0")}
                                    </div>
                                    <h4 className={`font-display text-base font-bold mb-1 ${node.labelColor || "text-primary"}`}>{node.label}</h4>
                    ...
                    >
                        ...
                        <div className="rounded-xl border border-border overflow-hidden bg-muted/5">
                            <ReactQuill
                                ...
                                formats={['header', 'bold', 'italic', 'underline', 'strike', 'size', 'list', 'bullet', 'color', 'background', 'link']}
                            />
                        </div>
                        ...
                        <div className="flex justify-end gap-3 pt-2 border-t border-border">
                            ...
                            <button onClick={handleSaveEdit} className="btn-primary px-7 py-2 text-sm">
                                Save Changes
                            </button>
                        </div>
                    </motion.div>
...
function AISummaryTab() {
    ...
    const fetchSummary = async () => {
        const pid = localStorage.getItem("current_paper_id");
        if (!pid) {
            toast({ title: "No Paper", description: "Upload a paper first to generate a summary.", variant: "destructive" });
            return;
        }
        setLoading(true);
        try {
            ...
            setSummary(data.summary || "No summary available yet. Try regenerating from the Upload page.");
        } catch {
    ...
    return (
        <div className="space-y-4 max-w-3xl mx-auto">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="font-display font-bold text-foreground text-lg">AI Summary</h2>
                    {paperTitle && <p className="text-muted-foreground text-xs mt-0.5 truncate max-w-md">{paperTitle}</p>}
                </div>
                <button
            ...
            {loading ? (
                ...
                <motion.div className="card-premium p-6 space-y-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    ...
                    {gfgTopics.length > 0 && (
                        <div className="p-3 rounded-xl bg-green-500/5 border border-green-500/15 space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {gfgTopics.map(t => <GFGLink key={t} topic={t} />)}
                            </div>
                        </div>
                    )}
              ...
              [&_code]:bg-muted [&_code]:px-1 [&_code]:rounded [&_code]:text-xs"
                    ...
                    <div className="pt-2 border-t border-border">
                        <button
                            onClick={() => { navigator.clipboard.writeText(summary); toast({ title: "Copied to clipboard" }); }}
                ...
                <div className="card-premium p-16 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">Upload a paper, then click Refresh to generate your summary.</p>
                </div>
            )}
        </div>
    );
...
export default function NotesPage() {
    ...
    const tabs = [
        { key: 'notes' as const, label: '📝 AI Notes', hint: 'Flowchart · Write · Save · Export' },
        { key: 'summary' as const, label: '🧠 AI Summary', hint: 'Simplified paper overview' },
    ];
